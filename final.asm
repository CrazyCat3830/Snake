...
DATASEG
	...
	starting_screen_text db "		[ By Danny Liberman ]", 10,10, "		Your goal is to move the snake, eat", 10,  "		fruits and get 20 points.", 10, "		If you touch the walls or yourself,", 10, "		you'll lose the game!",10,10,10, "		- Press a key to continue -$"
	...
	score_text db 13,"$"
	win_screen_text db "		[ You won! ]", 10,10, "You managed to get 20 points!  Good job!", 10,10,10, "- Press a key to exit -$"
	LOSE_screen_text db  10,10, "Better luck next time...", 10,10,10, "- Press a key to exit -$"
	;-----melodys and sounds-----;
	;rickrol copied from Ohad K (with promition)
	win_melody dw 2711, 2415, 2031, 2711, 1612, 1,1, 1612, 1,1, 1809, 1,1,1,1, 2711, 2415, 2031, 2711,  0; rip 1809, 1,1, 1809, 1,1, 2031, 1, 2152, 2415, 1,1, 2711, 2415, 2031, 2711, 2031, 1,1,1, 1809, 1, 2152, 1,1,1, 2711, 1,1, 2711, 1,1, 1809, 1,1, 2031, 0
	lose_melody dw 9663, 9121, 8126, 1, 12175, 1, 10847, 1, 13666, 1, 14478, 0
	score_melody dw 2415, 0
	eating_melody dw 1724, 0
...
snakepy: ;for each x value, we will print 11 diffferent pixels in 11 different y values
...
applepy: ;for each x value, we will print 9 diffferent pixels in 9 different y values
...
proc draw_fruit; rip not working :(
	push si
    push bp
    mov bp, sp
    mov si, offset appleSprite
    push ax
    push bx
    push cx
    push dx
...
notlast:
	loop sna_upd
	call check_snake ;check if head is valid
...
noValid:
	...
	ret
endp updateSnake
;-----checks if snakes cords are legal-----;
proc check_snake
	push ax
	push bx
	push cx
